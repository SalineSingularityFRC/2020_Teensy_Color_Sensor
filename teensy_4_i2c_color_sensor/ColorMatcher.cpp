...
double ColorMatcher::distance3d(double *pointOne, double *pointTwo){
    ...
    double point1[3] = pointOne;
    double point2[3] = pointTwo;
...
int ColorMatcher::matchColor(double *color){
    ...
    double closest = distance3d(colorTarget[0][0], *color);
    int index
    for(int r = 0; r < colorTarget.length; r++){
        if( (distance3d(colorTarget[r], *color)) < closest){
          closest = colorTarget[r];
          index = r;
      }
    }
    return index;
